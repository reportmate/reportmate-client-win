...
The ReportMate Windows client (`managedreportsrunner.exe`) collects system information using osquery and transmits it to the ReportMate API. The client is a self-contained .NET 8 executable that can be deployed via:
...
$action = New-ScheduledTaskAction -Execute "C:\Program Files\ReportMate\managedreportsrunner.exe"
...
```powershell
# Check installation
Test-Path "C:\Program Files\ReportMate\managedreportsrunner.exe"

# View version
& "C:\Program Files\ReportMate\managedreportsrunner.exe" version
